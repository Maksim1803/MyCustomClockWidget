package com.example.mycustomclockwidget
...
class ClockView @JvmOverloads constructor(context: Context, attributeSet: AttributeSet? = null) : View(context, attributeSet) {
    ...
    private var dashColor = Color.DKGRAY
    private var digitColor = Color.DKGRAY
    private var arrowColor = Color.DKGRAY
    ...
    private fun drawClock(canvas: Canvas) {
        ...
        for (number in numerals) {
            ...
            val x = (cos(angle) * radius * digitOffset - rect.width() / 2).toFloat()
            val y = (sin(angle) * radius * digitOffset + rect.height() / 2).toFloat()
